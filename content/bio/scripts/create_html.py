...
def create_book_block(key, book):
    ...
    month_ind = (
        int(book["personal"]["finish_month"])
        if "finish_month" in book["personal"]
        else 0
    )
...
with open("books.json", "r") as fr:
    ...
    recent_books = [
        b[0]
        for b in books
        if (date_getter(b[1]) >= 100 * now.year)
        or (
            (date_getter(b[1]) >= 100 * (now.year - 1))
            and date_getter(b[1]) % 100 >= now.month
        )
    ]
    old_books = [b for b in books if b[0] not in recent_books]
    ...
    with open("recent_books.generated.html", "w") as p:
        p.write(books_html)
