...
with open("poems.json", "r+") as fwrite:
    ...
    for name, poem_metadata in poems.items():
        if (
            (format_all and "rawpath" in poem_metadata)
            or ("body" not in poem_metadata)
            or (name in target)
        ):
            print(
                'Auto-formatting "'
